...
struct mat4 {
     ...
     */
    mat4(float v00, float v01, float v02, float v10, float v11, float v12, float v20, float v21, float v22);
     ...
     */
    float& operator()(std::uint8_t row, uint8_t column) { return (&columns[column].x)[row]; }
     ...
     */
    const float& operator()(std::uint8_t row, uint8_t column) const { return (&columns[column].x)[row]; }
     ...
     */
    vec4& operator[](std::uint8_t column) { return columns[column]; }
     ...
     */
    const vec4& operator[](std::uint8_t column) const { return columns[column]; }
